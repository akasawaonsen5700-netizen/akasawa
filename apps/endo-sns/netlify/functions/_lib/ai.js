...
async function generateDraftWithGemini(input, classification) {
  ...
  try {
    ...
    - 「現代人の疲れ、忙しさ」などの現状の悩みに強くフォーカスし、「この動画に現状の悩みや問題の解決のヒントがある」と伝えてください。
    - テロップ（narration）のコピペは厳禁です。
    - ハッシュタグはスパムにならないよう、本当に最適なものを厳選して3〜5個だけ末尾に付けてください。旅館名のタグは最後に1つだけ。
    - 最後にプロフィール紹介文を付与：「${BRAND.profileInstagram}」

    ■ X用の「text」（ポスト文）の書き方
    - 必ず遠藤正俊本人の語り口調（私）にしてください。テロップ（narration）のコピペは厳禁です。
