...
async function generatePlan() {
  ...
  try {
    ...
    // レンダリング
    el.marketAnalysis.innerHTML = parseMarkdown(data.marketAnalysis);
    el.pricingStrategy.textContent = data.pricingStrategy;
    el.planName.textContent = data.planName;
    el.planCatch.textContent = data.catchCopy;
    ...
    // 情緒的説明文の流し込み
    el.planDesc.value = data.description;
    el.planDescPreview.innerHTML = parseMarkdown(data.description);

    // 管理者向け設定シート
    const settings = data.otaSettings || {};
